...
public class PaymentErrorHandler {
    ...
    public ResponseEntity<Object> handleRateLimitError(CustomExcpts.RateLimitExceededException e) {
        ...
        response.put("message", buildRateLimitMessage(e));
        response.put("successful", false);
        response.put("errorCode", ListOfErrorCode.RATE_LIMIT_EXCEEDED.getCode());
    ...
    private ListOfErrorCode determineErrorCode(Throwable cause) {
        return switch (cause.getClass().getSimpleName()) {
            ...
            case "PaymentMethodNotActiveException" -> ListOfErrorCode.PAYMENT_METHOD_NOT_ACTIVE;
            case "PaymentMethodNotSupportedException" -> ListOfErrorCode.PAYMENT_METHOD_NOT_SUPPORTED;
            case "MobileNetworkOperatorNotActiveException" -> ListOfErrorCode.MOBILE_NETWORK_OPERATOR_NOT_ACTIVE;
    ...
    private Object createErrorResponse(ListOfErrorCode errorCode, Throwable cause) {
        ...
        errorResponse.setMessage(errorCode.getMessage() != null ? !cause.getMessage().isEmpty() ? errorCode.getMessage() : "Transaction processing failed" : "Transaction processing failed");
*/
        Map<String, Object> response = new HashMap<>();
        response.put("status", errorCode.getCode().startsWith("00") ? "SUCCESS" : "FAILED");
        response.put("message", errorCode.getMessage() != null ? !cause.getMessage().isEmpty() ? errorCode.getMessage() : "Transaction processing failed" : "Transaction processing failed");
        response.put("successful", false);
